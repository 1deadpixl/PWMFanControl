# Originally created by Michael Klements
# https://github.com/mklements/PWMFanControl

# Updated to use NumPy math functions

import RPi.GPIO as IO          # Calling GPIO to allow use of the GPIO pins
import time                    # Calling time to allow delays to be used
import subprocess              # Calling subprocess to get the CPU temperature
import numpy as np             # Calling NumPy for math functions

IO.setwarnings(False)          # Do not show any GPIO warnings
IO.setmode (IO.BCM)            # BCM pin numbers - PIN8 as ‘GPIO14’
IO.setup(14,IO.OUT)            # Initialize GPIO14 as our fan output pin
fan = IO.PWM(14,100)           # Set GPIO14 as a PWM output, with 100Hz frequency (this should match your fans specified PWM frequency)
fan.start(0)                   # Generate a PWM signal with a 0% duty cycle (fan off)

# Temperature and speed steps, edit these to define your temp thresholds and speed for each
temp_steps = [0, 25, 32, 40, 50, 60, 70]
speed_steps = np.linspace(0, 100, len(steps)) # List of evenly-spaced speeds
# speed_steps = [0, 15, 25, 50, 70, 85, 100] # OR define your own list of speeds (must be the same length as the temp steps)

if (len(temp_steps) != len(speed_steps)):
    raise RuntimeError("List of temperatures and speeds aren't the same length")

def get_temp():                              # Function to read in the CPU temperature and return it as a float in degrees celcius
    output = subprocess.run(['vcgencmd', 'measure_temp'], capture_output=True)
    temp_str = output.stdout.decode()
    try:
        return float(temp_str.split('=')[1].split('\'')[0])
    except (IndexError, ValueError):
        raise RuntimeError('Could not get temperature')

while 1:                                     # Execute loop forever
    temp = get_temp()                        # Get the current CPU temperature

    # Select fan speed based on the current temp threshold
    new_duty_cycle = speed_steps[temp_steps.index(list(filter(lambda x: x < temp, temp_steps))[-1])]

    fan.ChangeDutyCycle(new_duty_cycle)      # Set fan speed based on latest temperature reading
    time.sleep(5)                            # Sleep for 5 seconds
